...
当前工程实现已将 `oled_show()` 固定为赛题专用显示页：先清空 `OLED_GRAM`，再显示 `Contest Diff`、当前任务号、执行时间，以及 `Short: select` / `Long : start` 提示。底盘固定为两轮差速车，`MotorD` 作为左轮、`MotorA` 作为右轮；短按只在菜单态切换任务，长按只在菜单态启动当前任务。
...
当前工程将任务 1 临时用作两轮差速底盘的电机调试任务，并作为上电默认选中任务。该任务只给 MotorD 左轮、MotorA 右轮下发固定同向目标线速度，不运行循迹、滚球、IMU 角度环，用于确认电机方向、编码器测速和电机速度 PI 参数是否合理。

| 状态 | 行为 |
|---|---|
| T1_READY | 上电默认显示 `Motor Debug` 页面，电机停止，等待长按启动 |
| T1_MOTOR_RUN | 长按后 MotorD/MotorA 同速旋转，目标线速度由 `CONTEST_TASK1_MOTOR_DEBUG_SPEED` 设置 |
| T1_STOP | 运行中再次长按强制停止，回到菜单/调试显示页 |

OLED 调试页显示内容：左轮 MotorD、右轮 MotorA 编码器换算后的线速度值、目标线速度、电机速度 PI 参数 `Velocity_KP` 和 `Velocity_KI`、运行时间。若实测 D/A 速度符号不一致或与目标方向相反，优先检查差速车编码器极性和 PWM 极性；若速度震荡或跟随慢，再调整 `Velocity_KP` / `Velocity_KI`。
...
| T2_STRAIGHT1 | 左右轮同速前进，积分左轮 MotorD、右轮 MotorA 编码器速度，里程达到 1.5m 后切入弯道；用 ICM20948 yaw 角做小比例直行纠偏 |
| T2_CURVE1 | 启动 9 路红外循迹外环，沿赛道顺时针通过半径 0.5m 的半圆；阶段切换以编码器积分半圆弧长为主，超时只作为保护 |
| T2_STRAIGHT2 | 出弯后以新的 yaw 作为直行基准，再按编码器里程直行 1.5m |
| T2_CURVE2 | 再次红外循迹通过第二个顺时针半圆；里程达到半圆弧长后直接停车，不再等待停车基准线宽线特征 |
| T2_STOP_AT_BASELINE | 编码器累计里程到达目标或阶段超时保护触发后清零目标速度，停止计时并在 OLED 保留总时间 |
| T2_FAIL_STOP | 循迹丢线超时等保护停车，锁定当次运行时间 |

任务 2 关键参数集中在 `BALANCE/contest_task.c` 顶部：直线里程 `1.50m`、直线保护超时 `8s`、半圆半径 `0.50m`、半圆弧长 `3.1416 * 0.50m`、调试基础速度 `0.28m/s`、弯道保护超时 `12s`。OLED 任务 3 的阶段切换和最终停车不再使用宽线/多路传感器同时触发，红外只负责循迹外环和丢线保护。顺时针方向最终由 9 路传感器顺序、黑线电平和 `TRACK_TURN_DIR` 符号共同决定，实车若反向需优先调整 `BALANCE/track_control.h` 中的方向符号或传感器排列。
