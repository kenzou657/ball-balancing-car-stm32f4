...
6. 公共循迹模块保留“运行时间窗口 + 多路传感器触发 + 强制停车时间”的特征停车能力，但 OLED 任务 3 / 赛题任务 2 已改为由任务状态机使用编码器里程 + 阶段超时停车。
...
公共循迹模块仍支持“特征停车”和“强制停车”，供需要用宽线/A/B 点特征的任务复用。OLED 任务 3 / 赛题任务 2 的整圈停车不再使用多路传感器同时触发，而是在 `BALANCE/contest_task.c` 的任务状态机内使用 MotorD/MotorA 编码器积分里程作为主要依据，并用阶段超时作为辅助保护。
...
| 任务 | 停车目标 | 参数特点 |
|---|---|---|
| 任务 2 | 回 A 点停车 | 以编码器里程到达一圈分段目标为主，直线/弯道分别设置阶段超时保护，不依赖宽线触发 |
| 任务 4 | 到 B 点停车 | 可继续使用时间窗口 + 宽线/终点特征；若 B 点特征不稳定，应改用编码器里程辅助 |
| 任务 5 | 回 A 点停车 | 当前合并任务仍可使用时间窗口 + 宽线特征，调试不稳时可参考任务 2 改为里程停车 |
