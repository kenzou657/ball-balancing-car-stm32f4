...
本工程基于 STM32F4 + FreeRTOS + Keil MDK，目标是完成电赛选拔赛题中的差速循迹、滚球稳定以及二者联动任务。本文只做组内进度同步，不作为入门教程。
...
- 任务 1：当前作为电机调试任务，验证左右轮方向、编码器和速度 PI。
- 任务 2：一圈回 A，主要验证差速循迹和停车判定。
- 任务 3：静态滚球稳定，中心 -> +5cm -> -5cm。
- 任务 4/5/6：按合并框架处理，即选择 AB/AA 循迹模式和滚球平衡点。
...
| `kf` | `BallControlParam` | IMU 加速度前馈 | 任务 3 先 0，移动联调再开 |
...
1. 任务 1：固定左右轮目标速度，确认电机方向、编码器方向、速度 PI。
2. 红外输入：显示 `line_mask`、`active_count`、`line_error`，确认黑线电平和 IR 顺序。
3. 任务 2 低速循迹：先只调循迹 PD，不开复杂停车；方向错先改 `TRACK_TURN_DIR`。
4. 停车判定：记录 A/B 到达时间，调宽线阈值、停车窗口和强制停车时间。
5. 舵机单独调试：确认 PC9 输出、机械中位、方向和安全限幅。
6. 摄像头协议：确认心跳、位置正负、丢球和超时保护。
7. 任务 3 静态滚球：先 P，再 D，最后小 I；前馈 `kf` 保持 0。
8. 任务 4：AB + 球居中，低速联调，优先保球不追速度。
9. 任务 5：AA + 球居中，逐步提高整圈速度。
10. 任务 6：AA + 非零平衡点，重点看目标换算和保持效果。
...
- 任务 4/5/6 不建议写三套重复状态机，当前思路是“循迹模式 + 平衡点”的合并框架。
